...
#define ASSERT_STATE(exp, act) \
  TEST_ASSERT_EQUAL_INT(static_cast<int>(exp), static_cast<int>(act))

// 记录型 listener,用于捕获 core 发出的效果
struct RecordingListener : public ITimerCoreListener {
  ...
  void onStateChanged(State, State to) override { stateChanges++; lastState = to; }
  void onTick(const TickInfo& info) override { ticks++; lastTick = info; }
  void onPrintSlip(const SlipData& s) override { slips++; lastSlip = s; }
  void onFeedback(Feedback fb) override { if (fb == Feedback::TimeUp) timeUps++; }
};
...
void test_stop_emits_slip_and_returns_idle(void) {
  core.setTime(1);
  core.start();
  for (int i = 0; i < 90; ++i) core.tick();  // 超时 30s
  core.stop();
  ASSERT_STATE(State::Idle, core.state());
