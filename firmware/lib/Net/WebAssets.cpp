...
async function command(kind){try{els.msg.textContent='';if(kind==='set-start'){await post({cmd:'set',minutes:Number(els.minutes.value)||1});await post({cmd:'start'})}else if(kind==='savewifi'){await post({cmd:'config',data:{ssid:els.ssid.value,pass:els.pass.value}});els.msg.textContent='WiFi saved. Device restarting.'}else if(kind==='status'){await status()}else{await post({cmd:kind})}}catch(e){els.msg.textContent=e.message}}
